...
- (id)initWithURL:(NSURL *)URL localDir:(NSString*)m3u8Dir port:(int)servicePort videoID:(NSString*)videoID error:(NSError **)error{
#define USERAGENT @"Mozilla/5.0 (iPhone; CPU iPhone OS 5_1_1 like Mac OS X) AppleWebKit/534.46 (KHTML, like Gecko) Version/5.1 Mobile/9B206 Safari/7534.48.3"
    ...
    return [self initWithString:str baseURL:URL.realBaseURL localDir:m3u8Dir port:servicePort videoID:videoID error:error];
}

- (id)initWithString:(NSString *)string baseURL:(NSURL *)baseURL localDir:(NSString*)m3u8Dir port:(int)servicePort videoID:(NSString*)videoID error:(NSError **)error {
    self.videoID = videoID;
    self.m3u8Dir = m3u8Dir;
    self.serviceProt = servicePort;
    if (NO == [string isExtendedM3Ufile]) {
        *error = [NSError errorWithDomain:@"VideoPlaylistModel" code:-998 userInfo:@{NSLocalizedDescriptionKey:@"The content is not a m3u8 playlist"}];
        return nil;
    }
    ...
    if (self = [super init]) {
        if ([string isMasterPlaylist]) {
            ...
            if (self.currentXStreamInf) {
                ...
                self.mainMediaPl = [[M3U8MediaPlaylist alloc] initWithContentOfURL:m3u8URL type:M3U8MediaPlaylistTypeMedia localDir:self.m3u8Dir  port:self.serviceProt videoID:self.videoID error:&ero];
            ...
            if (list.count > 1) {
                for (int i = 0; i < list.count; i++) {
                    ...
                    if (xsinf.codecs.count == 1 && [xsinf.codecs.firstObject hasPrefix:@"mp4a"]) {
                        ...
                        self.audioPl = [[M3U8MediaPlaylist alloc] initWithContentOfURL:audioURL type:M3U8MediaPlaylistTypeAudio localDir:self.m3u8Dir port:self.serviceProt videoID:self.videoID error:NULL];
            ...
            self.baseURL = baseURL;
            self.mainMediaPl = [[M3U8MediaPlaylist alloc] initWithContent:string type:M3U8MediaPlaylistTypeMedia baseURL:self.baseURL localDir:self.m3u8Dir port:self.serviceProt videoID:self.videoID];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (URL.absoluteString.length > 0
            ...
            if ([URL.absoluteString isEqualToString:self.mainMediaPl.originalURL.absoluteString]) {
                ...
                NSError *error;
                M3U8MediaPlaylist *pl = [[M3U8MediaPlaylist alloc] initWithContentOfURL:URL type:M3U8MediaPlaylistTypeMedia localDir:self.m3u8Dir port:self.serviceProt videoID:self.videoID error:&error];
                if (pl) {
                    self.mainMediaPl = pl;
                    M3U8ExtXStreamInfList *list = self.masterPlaylist.xStreamList;
                    if (list.count > 1) {
