...
// VideoPlaylistModel 默认会按照《需要下载的内容》规则选取默认的playlist，如果需要手动指定获取特定的media playlist, 需调用方法 -specifyVideoURL:(这个在选取视频源的时候会用到);
...
- (id)initWithString:(NSString *)string baseURL:(NSURL *)URL localDir:(NSString*)m3u8Dir  port:(int)servicePort videoID:(NSString*)videoID error:(NSError **)error;
