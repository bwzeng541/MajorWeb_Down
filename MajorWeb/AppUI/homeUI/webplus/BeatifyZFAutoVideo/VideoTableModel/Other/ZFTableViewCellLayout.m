...
@property (nonatomic, assign) float deviceW;
...
- (instancetype)initWithData:(ZFTableData *)data {
    ...
    if (self) {
        ...
        self.deviceW = deviceSize.width>deviceSize.height?deviceSize.height:deviceSize.width;
        CGFloat min_x = 0;
        CGFloat min_y = 0;
        CGFloat min_w = 0;
        CGFloat min_h = 0;
        CGFloat min_view_w =  self.deviceW;
        CGFloat margin = 10;
...
- (CGFloat)videoHeight {
    ...
    if (self.isVerticalVideo) {
        videoHeight =  self.deviceW * 0.6 * self.data.video_height/self.data.video_width;
    } else {
        videoHeight = self.deviceW * self.data.video_height/self.data.video_width;
    }
    return videoHeight;
}
