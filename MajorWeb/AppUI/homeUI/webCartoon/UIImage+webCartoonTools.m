...
+(UIImage*)imageScale:(UIImage*)image scale:(float)scale quality:(CGInterpolationQuality)quality{
    ...
    @autoreleasepool {
        ...
        CGColorSpaceRef colorspaceRef = [SDWebImageImageIOCoder  colorSpaceForImageRef:sourceImageRef];
...
+ (UIImage*)drawImageAddRect:(CGSize)destResolution  rect:(CGRect)rect image:(UIImage*)addImage maskRect:(CGRect)maskRect maskImage:(UIImage*)maskImage{
    UIImage *img = nil;
        ...
        CGContextRef destContext = CGBitmapContextCreate(NULL,
            ...
            if (maskImage) {
                CGContextDrawImage( destContext, maskRect, maskImage.CGImage );
            }
        CGImageRef destImageRef = CGBitmapContextCreateImage(destContext);
            CGContextRelease(destContext);
            if (destImageRef) {
