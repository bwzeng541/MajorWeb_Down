...
@implementation PicUpIconsView(VideoNativeAd)
-(void)startVideoNative{
    if ([VipPayPlus getInstance].systemConfig.vip!=General_User || GetAppDelegate.isWatchHomeVideo) return;
...
-(void)addAdInView{
    if (!adVideoTableView) {
        ...
        adVideoCloseBtn.frame = CGRectMake(20, 20+offsetFixY, 22, 22);
        UIView *view =   [[[UIApplication sharedApplication] keyWindow] currentViewController].view;
        [view addSubview:adVideoTableView];
        [view addSubview:adVideoCloseBtn];
...
-(BOOL)isVaild:(NSString*)key{
    return true;
    return [[VipPayPlus getInstance] isVaildOperation:false plugKey:key];
}
@end
