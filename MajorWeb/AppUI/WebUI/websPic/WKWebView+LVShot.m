...
static MBProgressHUD *progressHub = nil;
...
-(void)shotScreenContentScrollCapture:(void(^)(UIImage*screenShotImage))completion{
    ...
    self.isShoting = YES;
    if (!progressHub) {
        progressHub = [MBProgressHUD showHUDAddedTo:[UIApplication sharedApplication].keyWindow animated:YES];
        progressHub.mode = MBProgressHUDModeDeterminate;
        progressHub.label.text = @"图片合成中,请稍后";
    }
    UIView *snapShotVIew = [self snapshotViewAfterScreenUpdates:YES];
