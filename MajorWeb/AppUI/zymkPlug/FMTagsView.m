...
- (void)updateSelectState:(BOOL)isSelect index:(NSInteger)index{
    [self deSelectAll];
      [self.tagModels objectAtIndex:index].selected = isSelect;
...
- (void)deSelectAll {
    for (int i = 0; i < self.tagModels.count; i++) {
        FMTagModel *tagModel = self.tagModels[i];
        tagModel.selected = NO;
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tagModel.selected) {
        ...
        if (!self.allowEmptySelection && self.collectionView.indexPathsForSelectedItems.count == 1) {
             if ([self.delegate respondsToSelector:@selector(tagsView:didSelectAlreadyTagAtIndex:)]) {
                [self.delegate tagsView:self didSelectAlreadyTagAtIndex:indexPath.row];
            }
            return;
        }
