...
- (void)sendButtonPressed:(UIButton *)button {
    //修改文字
    //printf("self.inputToolBar.inputTextView.text = %s",[self.inputToolBar.inputTextView.text UTF8String]);
    NSString *str = @"";//((AppDelegate*)[UIApplication sharedApplication].delegate).feedBackPrestr;
    if (str==nil) {
        str = @"";
    }
    self.inputToolBar.inputTextView.text = [NSString stringWithFormat:@"%@%@",str,self.inputToolBar.inputTextView.text];
    //end
    if (self.inputToolBar.isEditMode) {
        [self setIsEditMode:NO];
