      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    ...
    if ([delegate respondsToSelector:@selector(zipArchiveMajorProgressEvent:total:)]) {
        [delegate zipArchiveMajorProgressEvent:currentPosition total:fileSize];
    }
    ...
    do {
        ...
        @autoreleasepool {
            ...
            if ([delegate respondsToSelector:@selector(zipArchiveMajorProgressEvent:total:)]) {
                [delegate zipArchiveMajorProgressEvent:(NSInteger)currentPosition total:(NSInteger)fileSize];
            }
            ...
            // Message delegate
            if ([delegate respondsToSelector:@selector(zipArchiveMajorDidUnzipFileAtIndex:totalFiles:archivePath:fileInfo:)]) {
                [delegate zipArchiveMajorDidUnzipFileAtIndex:currentFileNumber totalFiles:(NSInteger)globalInfo.number_entry
                                            archivePath:path fileInfo:fileInfo];
            } else if ([delegate respondsToSelector: @selector(zipArchiveMajorDidUnzipFileAtIndex:totalFiles:archivePath:unzippedFilePath:)]) {
                [delegate zipArchiveMajorDidUnzipFileAtIndex: currentFileNumber totalFiles: (NSInteger)globalInfo.number_entry
    ...
    if (!canceled && [delegate respondsToSelector:@selector(zipArchiveMajorProgressEvent:total:)]) {
        [delegate zipArchiveMajorProgressEvent:fileSize total:fileSize];
    }
...
#pragma mark - Zipping
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray<NSString *> *)paths delegate:(nullable id<SSZipCreateArchiveDelegate>)delegate
{
    return [SSZipArchive createZipFileAtPath:path withFilesAtPaths:paths withPassword:nil delegate:delegate];
}
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath delegate:(nullable id<SSZipCreateArchiveDelegate>)delegate{
    return [SSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath withPassword:nil delegate:delegate];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory delegate:(nullable id<SSZipCreateArchiveDelegate>)delegate{
    return [SSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory withPassword:nil delegate:delegate];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray<NSString *> *)paths withPassword:(NSString *)password delegate:(nullable id<SSZipCreateArchiveDelegate>)delegate
{
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath withPassword:(nullable NSString *)password delegate:(nullable id<SSZipCreateArchiveDelegate>)delegate{
    return [SSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:NO withPassword:password delegate:delegate];
}


+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory withPassword:(nullable NSString *)password delegate:(nullable id<SSZipCreateArchiveDelegate>)delegate{
    return [SSZipArchive createZipFileAtPath:path
                     withContentsOfDirectory:directoryPath
                         keepParentDirectory:keepParentDirectory
                          ...
                          andProgressHandler:nil delegate:delegate
            ];
}
...
+ (BOOL)createZipFileAtPath:(NSString *)path
    withContentsOfDirectory:(NSString *)directoryPath
        keepParentDirectory:(BOOL)keepParentDirectory
         ...
         andProgressHandler:(void(^ _Nullable)(NSUInteger entryNumber, NSUInteger total))progressHandler delegate:(nullable id<SSZipCreateArchiveDelegate>)delegate{
    return [self createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory compressionLevel:Z_DEFAULT_COMPRESSION password:password AES:YES progressHandler:progressHandler delegate:delegate];
}
...
+ (BOOL)createZipFileAtPath:(NSString *)path
    withContentsOfDirectory:(NSString *)directoryPath
        keepParentDirectory:(BOOL)keepParentDirectory
           compressionLevel:(int)compressionLevel
                   password:(nullable NSString *)password
                        AES:(BOOL)aes
            progressHandler:(void(^ _Nullable)(NSUInteger entryNumber, NSUInteger total))progressHandler delegate:(nullable id<SSZipCreateArchiveDelegate>)delegate{
    ...
    if (success) {
        ...
        for (fileName in allObjects) {
            ...
            if (delegate && [delegate respondsToSelector:@selector(zipCanArchive:)]) {
                isCanWrite = [delegate zipCanArchive:fullFilePath];
            }
            if (isCanWrite) {
                if (!isDir) {
