...
+ (id)downloadWebImageWithUrl:(NSURL *)url referer:(NSString*)referer options:(SDWebImageDownloaderOptions)options progress:(YBImageBrowserDownloaderProgressBlock)progress success:(YBImageBrowserDownloaderSuccessBlock)success failed:(YBImageBrowserDownloaderFailedBlock)failed {
    if (!url) return nil;
    
    SDWebImageDownloadToken *token = [self.downloader downloadImageWithURL:url refer:referer options:options progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
