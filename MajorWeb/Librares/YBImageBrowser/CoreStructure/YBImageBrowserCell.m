...
- (void)removeAllGestureAndNotification{
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    if (self.tapDouble) {
        [self.scrollView removeGestureRecognizer:self.tapDouble];
        self.tapDouble = nil;
    }
    if (self.longPress) {
        [self.scrollView removeGestureRecognizer:self.longPress];
        self.longPress = nil;
    }
    if (self.tapSingle) {
        [self.scrollView removeGestureRecognizer:self.tapSingle];
        self.tapSingle = nil;
    }
    self.scrollView.userInteractionEnabled = NO;
}

-(void)addAllGestureAndNotification
{
    [self removeAllGestureAndNotification];
    [self addGesture];
    [self addNotification];
...
- (void)addGesture {
    if(!self.tapSingle){
        self.tapSingle = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(respondsToTapSingle:)];
        self.tapSingle.numberOfTapsRequired = 1;
        [self.scrollView addGestureRecognizer:self.tapSingle];
    }
    if(!self.tapDouble){
        self.tapDouble = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(respondsToTapDouble:)];
    ...
    if(!self.longPress){
        self.longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(respondsToLongPress:)];
...
- (void)loadImageWithModel:(YBImageBrowserModel *)model isPreview:(BOOL)isPreview {
    ...
    if (model.image) {
        ...
        if ([self.delegate respondsToSelector:@selector(addMaskByYBImageBrowserCell:)]) {
            [self.delegate addMaskByYBImageBrowserCell:self];
        }
