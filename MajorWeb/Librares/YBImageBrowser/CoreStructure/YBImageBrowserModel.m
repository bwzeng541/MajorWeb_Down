...
-(void)cancelTaskWithDownloadToken{
    if (downloadToken) {
...
- (void)dealloc {
    [self cancelTaskWithDownloadToken];
}
...
- (void)downloadImageProgress:(YBImageBrowserModelDownloadProgressBlock)progress success:(YBImageBrowserModelDownloadSuccessBlock)success failed:(YBImageBrowserModelDownloadFailedBlock)failed {
    ...
    downloadToken = [YBImageBrowserDownloader downloadWebImageWithUrl:model.url referer:model.referer options:SDWebImageDownloaderLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
...
- (void)scaleImageWithCurrentImageFrame:(CGRect)imageFrame complete:(YBImageBrowserModelScaleImageSuccessBlock)complete {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        model.image = [YBImageBrowserUtilities scaleToSizeWithImage:self->largeImage size:imageFrame.size];
        if (complete) {
            YB_MAINTHREAD_ASYNC(^{
                complete(model);
            })
...
- (void)cutImageWithTargetRect:(CGRect)targetRect complete:(YBImageBrowserModelCutImageSuccessBlock)complete {
    YBImageBrowserModel *model = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        UIImage *resultImage = [YBImageBrowserUtilities cutToRectWithImage:self->largeImage rect:targetRect];
        if (complete) {
            YB_MAINTHREAD_ASYNC(^{
