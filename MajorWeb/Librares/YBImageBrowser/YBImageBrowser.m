...
- (void)addMaskByYBImageBrowserCell:(YBImageBrowserView*)imageBrowserView imageBrowserCell:(YBImageBrowserCell *)imageBrowserCell {
    if ([self.dataSource respondsToSelector:@selector(addMaskByYBImageBrowserCell:imageBrowserCell:)]) {
        [self.dataSource addMaskByYBImageBrowserCell:self imageBrowserCell:imageBrowserCell ];
    }
}
...
- (void)savePhotoToAlbumWithCurrentIndex {
    ...
    if (cell.model){
        ...
        if(self.delegate && [self.delegate respondsToSelector:@selector(yBImageBrowserIsCanExcel:clickFunctionWithName:imageModel:)]){
            if ([self.delegate yBImageBrowserIsCanExcel:self clickFunctionWithName:YBImageBrowserFunctionModel_ID_savePictureToAlbum imageModel:cell.model]) {
                [self savePhotoToAlbumWithModel:cell.model preview:NO];
            }
            else{
