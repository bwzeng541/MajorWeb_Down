...
#endif
        if (success) {
            if ([self->_delegate respondsToSelector:@selector(dataMajorbasePool:shouldMajorToPool:)] && ![self->_delegate dataMajorbasePool:self shouldMajorToPool:db]) {
                [db close];
                db = 0x00;
            }
            else {
                ...
                if (![self->_databaseOutPool containsObject:db]) {
                    ...
                    if (shouldNotifyDelegate && [self->_delegate respondsToSelector:@selector(dataMajorbasePool:didMarjorAddDatabase:)]) {
                        [self->_delegate dataMajorbasePool:self didMarjorAddDatabase:db];
                    }
                }
            }
        }
