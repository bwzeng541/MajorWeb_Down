...
extern Protocol *a2_delegateUserProtocol(Class cls,NSString *suffix);
...
static Protocol *a2_protocolForDelegatingObject(id obj, Protocol *protocol)
{
	...
	if ([protocolName hasSuffix:@"Delegate"]) {//
        Protocol *p =NULL;
        if ([protocolName compare:@"WKNavigationDelegate"]==NSOrderedSame) {
            p  = a2_delegateUserProtocol([obj class],protocolName);
        }
        else{
          p  = a2_delegateProtocol([obj class]);
        }
		if (p) return p;
...
+ (void)bk_linkDelegateUesrMethods:(NSDictionary *)dictionary suffix:(NSString*)name{
    [self bk_linkProtocol:a2_delegateUserProtocol(self, [name  stringByAppendingString:@"Delegate"]) methods:dictionary];
}
...
+ (void)bk_registerDynamicDelegateUesrNamed:(NSString *)delegateName suffix:(NSString*)name{

    [self bk_registerDynamicDelegateNamed:delegateName forProtocol:a2_delegateUserProtocol(self, [name  stringByAppendingString:@"Delegate"])];
}
