...
+ (NSString *)getCacheSize {
    NSString *cachesPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject];//[ClearCachesTool getFileDirSize:[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject]]
    NSInteger totleSize = [ClearCachesTool getFileDirSize:cachesPath] ;
    return [ClearCachesTool convertByteToShort:totleSize];
}
...
+ (NSInteger) getFileDirSize:(NSString*)path{
    NSArray *subPathArr = [[NSFileManager defaultManager] subpathsAtPath:path];
    NSString *filePath  = nil;
    NSInteger totleSize = 0;
    for (NSString *subPath in subPathArr){
        ...
        if (![subPath containsString:@"Snapshots"]&&![subPath containsString:@"SynchronizationDir"]&&![filePath containsString:@"zfautolist"]&&![subPath containsString:@"WebAdCache"]&&![subPath containsString:@"zymywww"]) {
            // 1. 拼接每一个文件的全路径
            filePath =[path stringByAppendingPathComponent:subPath];
        }
        // 2. 是否是文件夹，默认不是
...
+ (NSString *)convertByteToShort:(NSInteger)totleSize{
    NSString *totleStr = nil;
    if (totleSize > 1000 * 1000){
        totleStr = [NSString stringWithFormat:@"%.2fM",totleSize / 1000.00f /1000.00f];
    }else if (totleSize > 1000){
...
+ (NSString *)getCacheSizeWithFilePath:(NSString *)path {
    ...
    // 获取“path”文件夹下的所有文件
    NSInteger totleSize = [self getFileDirSize:path];
    //8. 将文件夹大小转换为 M/KB/B
    NSString *totleStr = [self convertByteToShort:totleSize];
    return totleStr;
}
...
+ (BOOL)clearCacheWithFilePath:(NSString *)path{
    ...
    for (NSString *subPath in subPathArr) {
        filePath = [path stringByAppendingPathComponent:subPath];
        //        ZYLog(@"%@",filePath);
        if (![filePath containsString:@"/Caches/helpCaches"]&&![filePath containsString:@"/Caches/Snapshots"]&&![filePath containsString:@"/Caches/SynchronizationDir"]&&![filePath containsString:@"/VideoDownCaches"]&&![filePath containsString:@"zfautolist"]&&![subPath containsString:@"WebAdCache"]&&![filePath containsString:@"bu_template"]&&![filePath containsString:@"com.bytedance.bumedia"]&&![filePath containsString:@"zymywww"]) {
            //删除子文件夹
            [[NSFileManager defaultManager] removeItemAtPath:filePath error:&error];
        }
        if (error) {
