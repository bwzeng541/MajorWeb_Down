...
- (void)setRr_isActive:(BOOL)rr_isActive{
    _rr_isActive = rr_isActive;
    if ([self.rr_collectionView.rr_swipeActionDelegate conformsToProtocol:@protocol(RRSwipeActionDelegate)] ||
        [self.rr_collectionView.rr_swipeActionDelegate respondsToSelector:@selector(rr_collectionViewEditChange:eidtState:)]) {
        [self.rr_collectionView.rr_swipeActionDelegate rr_collectionViewEditChange:self.rr_collectionView eidtState:rr_isActive];
    }
}
...
- (void)_rr_handlePan:(UIPanGestureRecognizer *)sender {
    ...
    switch (sender.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            self.rr_isActive = x>0?YES:false;
        } break;
