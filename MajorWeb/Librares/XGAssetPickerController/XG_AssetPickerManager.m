...
- (void)getAllVideo:(void (^)(NSArray<XG_AlbumModel *> *models))completion{
            ...
            if ([obj.localizedTitle isEqualToString:@"相机胶卷"] || [obj.localizedTitle isEqualToString:@"Camera Roll"]) {
               // [albumArr insertObject:[self modelWithResult:fetchResult name:obj.localizedTitle videoPickable:videoPickable] atIndex:0];
                ...
                if (obj.assetCollectionSubtype == PHAssetCollectionSubtypeSmartAlbumVideos) {
                    [albumArr addObject:[self modelWithResult:fetchResult name:obj.localizedTitle videoPickable:true]];
                }
            }
        }
    }];
...
- (XG_AlbumModel *)modelWithResult:(PHFetchResult *)result name:(NSString *)name videoPickable:(BOOL)videoPickable{
    ...
    for (PHAsset *asset in result) {
        BOOL isAdd = true;
        if (self.delagte && [self.delagte respondsToSelector:@selector(xg_picker_manager_isAssetVaild:)]) {
            isAdd = [self.delagte xg_picker_manager_isAssetVaild:asset];
        }
        if(isAdd)
