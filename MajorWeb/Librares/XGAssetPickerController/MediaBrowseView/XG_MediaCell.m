...
-(void)selectClick:(UIButton*)sender{
    _item.picked = !_item.picked;
    [self.selectBtn setImage:(_item.picked ? ImageWithFile(@"picker_selected") : ImageWithFile(@"picker_unselected")) forState:UIControlStateNormal];
    [self.syncDelegate syncItemState:self.item];
}

- (UIButton*)selectBtn{
    if (!_selectBtn) {
        _selectBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_selectBtn setImage:ImageWithFile(@"picker_unselected") forState:UIControlStateNormal];
...
- (void)setItem:(XG_AssetModel *)item{
    ...
    [self.selectBtn setImage:(item.picked ? ImageWithFile(@"picker_selected") : ImageWithFile(@"picker_unselected")) forState:UIControlStateNormal];
...
- (void)resizeSubviewSize {
    ...
    self.selectBtn.frame = CGRectMake(_mediaContainerView.bounds.size.width-35, kAppStatusBarHeight+5, 30, 30);
