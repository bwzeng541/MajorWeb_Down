...
@interface XG_AssetPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UITableViewDelegate,UITableViewDataSource,UINavigationControllerDelegate, UIImagePickerControllerDelegate,PHPhotoLibraryChangeObserver,XG_MediaBrowseViewDelegate>
...
-(void)setAssetArr:(NSMutableArray<XG_AssetModel *> *)assetArr{
    _assetArr = assetArr;
    //插入相机占位
    if ([self.delegate respondsToSelector:@selector(syncPickedAssetModels:)]) {
        self.pickerOptions.pickedAssetModels = [self.delegate syncPickedAssetModels:self.assetArr];
    }
...
-(void)syncSelectItem:(XG_AssetModel*)item{
    for (int i = 0; i < self.assetArr.count; i++) {
        XG_AssetModel *model = self.assetArr[i];
        if (model.asset && item.asset && [model.asset.localIdentifier compare:item.asset.localIdentifier]==NSOrderedSame) {
            [self.assetArr replaceObjectAtIndex:i withObject:item];
            NSArray *array = [self.collectionView visibleCells];
            for (int i = 0; i < array.count; i++) {
              XG_AssetCell *cell =  [array objectAtIndex:i];
                if(cell.model.asset && [cell.model.asset.localIdentifier compare:item.asset.localIdentifier]==NSOrderedSame) {
                    [cell clickSelect];
                    return;
                }
            }
