...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		[sharedQueue setMaxConcurrentOperationCount:100];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setAllowCompressedResponse:YES]; //accpet-encoding : gzip 支持压缩  1096加了个,deflate
    ...
    //add zengbiwang
    [self setDidDownProgressSelector:@selector(request:DownProgress:)];
    [self setDidUpProgressSelector:@selector(request:UpProgress:)];
    //end
	[self setURL:newURL];
...
#endif
    NSArray *array = sharedQueue.operations;
    for  (int i = 0 ; i < [array count]; i++) {
        ASIHTTPRequest *temp = (ASIHTTPRequest*)[array objectAtIndex:i];
        if (temp == self) {
            return;
...
- (BOOL)startAsynchronousInWifi{
    //屏蔽3G网络 调用失败函数
    if(FALSE&&[[Reachability reachabilityForInternetConnection] isReachableViaWWAN]){
...
- (void)updateDownloadProgress
{
    ...
    if (downloadProgressDelegate && [downloadProgressDelegate respondsToSelector:@selector(request:didReceiveBytes:)]) {
        [downloadProgressDelegate request:self didReceiveBytes:value];
    }
    else{
    ...
    //delete this user delegate to upDataProgress
    ...
    if (isShowProgress) {
        ...
        if (delegate && [delegate respondsToSelector:didDownProgressSelector]) {
            NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:progressAmount],@"Progress",[NSNumber numberWithInt:progress],@"Bytes",nil];
        	[delegate performSelector:didDownProgressSelector  withObject:dic];
        }
    }
   //end
	#if NS_BLOCKS_AVAILABLE
...
- (void)updateUploadProgress
{
    ...
    if (isShowProgress) {
        unsigned long long progress = [self totalBytesSent]-[self uploadBufferSize];
        unsigned long long  total = [self postLength]-[self uploadBufferSize];
        float progressAmount = (float)((progress*1.0)/(total*1.0));
        if (delegate && [delegate respondsToSelector:didUpProgressSelector]) {
        	[delegate performSelector:didUpProgressSelector withObject:self withObject:[NSNumber numberWithFloat:progressAmount]];
        }
    }
    //end
	#if NS_BLOCKS_AVAILABLE
...
- (BOOL)willRedirect
{
    ...
    NSLog(@"asi redirect = %@",[responseHeaders description]);
    NSString *tmpDir = [responseHeaders valueForKey:@"Location"];
    NSCharacterSet *encode_set= [NSCharacterSet URLQueryAllowedCharacterSet];
    NSString *urlString_encode = [tmpDir stringByAddingPercentEncodingWithAllowedCharacters:encode_set];
	[self setRedirectURL:[[NSURL URLWithString:urlString_encode relativeToURL:[self url]] absoluteURL]];
...
- (void)handleBytesAvailable
{
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		// Are we downloading to a file?
			...
			NSInteger size = [[self fileDownloadOutputStream] write:buffer maxLength:bytesRead];
...
{
	NSError *err = nil;
	if (self.isAuToRemoveTmp && [self temporaryFileDownloadPath]) {
...
//add zengbiwang
@synthesize didDownProgressSelector;
@synthesize didUpProgressSelector;
//end
