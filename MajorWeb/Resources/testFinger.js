...
function triggerMoveFinger(){
    ...
    if(newpos>=0&&newpos<document.body.scrollHeight-window.screen.height && tirggerFingerCurrentDis<tirggerFingerTotalDis){
        ...
        clearInterval(intervalFingerTime);
        intervalFingerTime = 0;
        setTimeout('triggerEndOrLopp()',Math.random()*2000+1000);
...
function moveFingerWeb(){
    if(triggerFingerXloopTime==-1){
        if(triggerFingerNewsMode)
            triggerFingerXloopTime = triggerFingerGetRandValue(5,4);
        else
            triggerFingerXloopTime = triggerFingerGetRandValue(6,5);
    ...
    if(lefVar>=tirggerFingerTotalDis){
        ...
        {
            setTimeout(function(){ window.webkit.messageHandlers.ClickDelayTime.postMessage(null);}, 1000);
        }
        else{
...
function addTriggerClickATag(){//把所有符合添加的标签加进去
    ...
    var alinksTmp = filterAllUrlVaild();
    var alinks = new Array();
    for(var j = 0;j<alinksTmp.length;j++){
        v  = alinksTmp[j].href;
        if(v.indexOf('usrprofile?action=agreement&page=')!=-1 || v.indexOf('tel:')!=-1 ){
            // window.webkit.messageHandlers.ClickWebUrl.postMessage(v);
        }
        else{
            alinks.push(alinksTmp[j]);
        }
    }
    ...
    if(alinks.length>0){
        ...
        for(var ii = 0;ii<alinks.length;ii++){
            ...
            if(!isVi)//正常大小
            {
                ...
                if(height>12&&width>12){//一般这个大小才能点到
        ...
        if(triggerFingerAdUrlArray.length>0){//随机选择一个，还是指定一个？
            ...
            if(triggerFingerAdOperation){//需要传递参数给wkwebview，表示什么情况下刷新，
                ...
                if(triggerFingerMaxAdDelayTime==0){
                    moveFingerWeb();
                    triggerFingerMaxAdDelayTime =setInterval(function(){ window.webkit.messageHandlers.ClickDelayTime.postMessage(null);}, 5000);
                }
...
function triggerStartFrie(){
    if(triggerTotalCount==0){
        var b = triggerFingerGetRandValue(13,11);
        if(triggerFingerNewsMode){
            b = triggerFingerGetRandValue(8,6);
        }
